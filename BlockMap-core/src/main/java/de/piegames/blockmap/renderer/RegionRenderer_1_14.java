					...
					for (byte x = 0; x < 16; x++) {
						...
						height: for (byte s = 15; s >= 0; s--) {
							...
							for (int y = 15; y >= 0; y--) {
								...
								BlockColor colorData = settings.blockColors.getBlockColor(block.name, block.state);
	...
	private Block[] renderSection(CompoundMap section) {
		...
		List<Block> palette = ((ListTag<CompoundTag>) section.get("Palette"))
				...
				.map(map -> new Block(
						...
						parseBlockState((CompoundTag) map.get("Properties"), settings.version.getBlockStates())))
