		...
		{// Shading
			for (int z = 0; z < 512; z++)
				for (int x = 0; x < 512; x++) {
					...
					double gX = (northWestHeight + 2 * westHeight + southWestHeight - eastHeight - 2 * northEastHeight - southEastHeight);
					double gY = (northWestHeight + 2 * northHeight + northEastHeight - southWestHeight - 2 * southHeight - southEastHeight);
					...
					// Test 1
					// map[z << 9 | x] = Color.alphaOver(map[z << 9 | x], new Color((float) factor, 1, 1, 1));

					// Test 2
					// if ((height[z << 9 | x] & 7) == 0 && factor > 0.05)
